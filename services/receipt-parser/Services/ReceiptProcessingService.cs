...
public sealed class ReceiptProcessingService
{
    ...
    {
        var document = ReceiptDraftFactory.BuildReceiptDocument(parsed, uploadedByUserIdOverride);
        await _repository.SaveAsync(document, cancellationToken);

        var payload = ReceiptDraftFactory.BuildNotificationPayload(document);
